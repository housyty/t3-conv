...
| Tianzheng ARX 目录 | `sys23x64` |
| `tch_kernal.arx` 文件版本 | `6, 0, 404, 8` |
| AutoCAD 宿主 | AutoCAD 2020（当前 C++ 工具可自动检测 AutoCAD 2020-2026） |
| `acad.exe` 产品版本 | `R23.1.47.0.0` |
...
| 名称 | RVA | 说明 |
| --- | ---: | --- |
| `AcDbDatabase::ctor` | `0x64A8D0` | 构造数据库对象 |
| `AcDbDatabase::dtor` | `0x64A8D6` | 析构数据库对象 |
| `AcDbDatabase::readDwgFile` | `0x64A912` | 读取源 DWG |
| `sub_1C01B310` | `0x01B310` | T3 前置组/集合预处理 |
| `sub_1C01E850` | `0x01E850` | T3 前置块递归预处理入口 |
| `SaveAsTArch3` / `sub_1C027310` | `0x027310` | T3 专用保存 worker |
| selector base 全局值 | `0x9E6C7C` | UI 当前 CAD 版本索引；固定 AutoCAD 2004 时可不依赖它 |
| non-UI flag | `0x9F95B0` | 写入 `uint32_t 1` |
| UI state flag | `0x9E6C80` | 写入 `uint32_t 1` |
...
```text
RCX = db_remote
CALL tch_kernal_base + 0x01B310

RCX = db_remote
CALL tch_kernal_base + 0x01E850
```
...
但这只适合“复用当前全局选择”的实现，不适合对外承诺可重复结果的无 UI 服务。当前 C++ 实现会记录 `selector_base` 和最终 `selector`，并仍以 `selector=16` 作为 T3 + AutoCAD 2004 的成功验收条件。
...
```text
RCX = db_remote
CALL tch_kernal_base + 0x64A8D6
```

## 无 UI 标志

在执行远程 stub 前，建议写入以下两个进程内全局值：
...
| 确认对话框 | 远程 worker 固定路径、固定 selector，不走文件选择和保存确认 UI；同时写入两个 Tianzheng no-UI 标志。 | 设置 `FILEDIA=0`、`CMDDIA=0`、`EXPERT=5`，调用前删除旧输出，避免覆盖确认；超时后记录文件并进入下一张。 |
...
8. 读取控制块并判断 `status=1`、`read_status=0`、`save_result=5100`、`selector=16`。
